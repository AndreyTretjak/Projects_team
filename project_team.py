...
# Данные выгружает из папки: S:\\1. Приказы\\Приказы на команды проектов
...
folder_path = r'S:\\1. Приказы\\Приказы на команды проектов'
...
if combined_df is not None:
    ...
    # print("Объединенный DataFrame с приказами на команду проектов сохранен. Путь: r'S:\Приказы на команду проекта\project_team.xlsx")
    # combined_df.to_excel(
    #     r'S:\\Приказы на команду проекта\\project_team.xlsx', index=False)
else:
    print("Ни один из файлов не был успешно обработан.")


if check_obj_key_df is not None:
    path = "S:\\Приказы на команду проекта\\Проверка приказов\\check_obj_key_" + \
        datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + ".xlsx"
    print("Проверка корректности кодов ДС в приказах проведена. Результат сохранен. Путь: r'S:\\Приказы на команду проекта\\Проверка приказов\\check_obj_key.xlsx'")
